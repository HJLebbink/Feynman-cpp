...
namespace feynman {
	...
	public:
		...
		*/
			...
			std::mt19937 &rng)
		{
			...
			for (size_t vli = 0; vli < _visibleLayers.size(); vli++) {
				...
				vl._reconError = Array2D<float>(vld._size);
			}
			...
			randomUniform2D(_hiddenThresholds[_back], initThresholdRange, rng);
		}
		...
		*/
		...
		void reconstruct(
			...
			std::vector<Array2D<float>> &reconstructions)
		{
	...
	private:
		...
		static void scStimulus(
			const Array2D<float> &visibleStates, // was float2
...
#							pragma ivdep
		...
		static void scReverse(
			const Array2D<float> &hiddenStates,
			const Array2D<float> &visibleStates, // was float2
			Array2D<float> &reconErrors,		// write only
			const Array3D<float> &weights,
			const int2 visibleSize,
			const int2 hiddenSize,
...
#							pragma ivdep
		...
		static void scLearnWeights(
			...
			const Array2D<float> &reconErrors,
...
#						pragma ivdep
		...
		static void scLearnThresholds(
			const Array2D<float> &hiddenStates,
			const Array2D<float> &hiddenThresholdsBack,
			Array2D<float> &hiddenThresholdsFront, // write only
			const float thresholdAlpha,
			const float activeRatio,
			const int2 range)
		{
...
#					pragma ivdep
		...
		*/
		static void scDeriveInputs(
			...
			//const Array2D<float> /*&outputsBack*/, // unused: was float2
			Array2D<float> &outputsFront, // write only: was float2
...
#					pragma ivdep
		...
		static void scReconstruct(
			const Array2D<float> &hiddenStates,
			Array2D<float> &reconstruction, // write only
			const Array3D<float> &weights,
			const int2 /*visibleSize*/,
			const int2 hiddenSize,
			const float2 visibleToHidden,

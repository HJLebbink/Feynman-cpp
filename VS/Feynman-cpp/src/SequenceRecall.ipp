...
void sequenceRecall() {
	...
	Predictor ph;
	ph.createRandom({ 4, 4 }, pLayerDescs, layerDescs, { -0.01f, 0.01f }, generator);
	...
	while (numSequences > 0) {
		...
		for (size_t show_iter = 0; show_iter < digits.size(); show_iter++) {
			...
			copy(inputImage, digitVec);
			ph.simStep(inputImage, inputImage, generator);
		}
		...
		for (size_t recall_iter = 0; recall_iter < digits.size(); recall_iter++) {
			...
			copy(ph.getPrediction(), prediction);

...
namespace vis {
	...
	struct Plot {
		...
		void draw(
			...
			const sf::Texture &lineGradientTexture,
			const sf::Font &tickFont,
			const float tickTextScale,
			const sf::Vector2f &domain,
			const sf::Vector2f &range,
			const sf::Vector2f &margins,
			const sf::Vector2f &tickIncrements,
			const float axesSize,
			const float lineSize,
			const float tickSize,
			const float tickLength,
			const float textTickOffset,
			const int precision)
		{
			...
			// Draw curves
			for (int c = 0; c < static_cast<int>(_curves.size()); c++) {
				...
				// Go through points
				for (int p = 0; p < static_cast<int>(_curves[c]._points.size()) - 1; p++) {
					...
					//sf::Vector2f direction = vectorNormalize(difference); // unused
					...
					if (pointVisible || pointNextVisible) {
						...
						if (p < static_cast<int>(_curves[c]._points.size()) - 2) {
						...
						vertexArray[index].position = renderPointNext + sizeOffsetNext;
						vertexArray[index].texCoords = sf::Vector2f(0.0f, static_cast<float>(lineGradientTexture.getSize().y));
						vertexArray[index].color = pointNext._color;
				...
				if (_curves[c]._shadow != 0.0f) {
					...
					for (size_t v = 0; v < shadowArray.getVertexCount(); v++) {
						shadowArray[v].position += _curves[c]._shadowOffset;
						shadowArray[v].color = sf::Color(0, 0, 0, static_cast<sf::Uint8>(_curves[c]._shadow * 255.0f));
					}
			...
			leftMask.setSize(sf::Vector2f(margins.x, static_cast<float>(target.getSize().y)));
			leftMask.setFillColor(_backgroundColor);

			target.draw(leftMask);

			sf::RectangleShape rightMask;
			rightMask.setSize(sf::Vector2f(static_cast<float>(target.getSize().x), static_cast<float>(margins.y)));
			...
			// Draw ticks
			{
				...
				int xTicks = static_cast<int>(std::floor(xDistance / tickIncrements.x));
...

			{
				...
				int yTicks = static_cast<int>(std::floor(yDistance / tickIncrements.y));
